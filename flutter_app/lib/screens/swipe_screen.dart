...
class _SwipeScreenState extends State<SwipeScreen> {
  ...
  Future<void> _onSwipe(int previousIndex, int? currentIndex, CardSwiperAction action) async {
    ...
    final targetUserId = user['_id'] ?? user['id'];

    if (action == CardSwiperAction.right) {
      final isMutual = await APIService.likeUser(targetUserId);
      if (isMutual && mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text("?? It's a match!"), backgroundColor: Colors.green),
        );
      }
  ...
  Widget build(BuildContext context) {
        ...
        body: Center(
          child: Column(
            ...
            children: [
              ...
              ElevatedButton(onPressed: _loadMatches, child: const Text('Refresh')),
            ],
          ),
        ),
      );
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('Meengle'), centerTitle: true),
      body: Column(
        children: [
          Expanded(
            child: CardSwiper(
          ...
          Padding(
            ...
            child: Row(
              ...
              children: [
                FloatingActionButton(heroTag: 'pass', onPressed: () => _controller.swipeLeft(), backgroundColor: Colors.red, child: const Icon(Icons.close)),
                FloatingActionButton(heroTag: 'like', onPressed: () => _controller.swipeRight(), backgroundColor: Colors.green, child: const Icon(Icons.favorite)),
              ],
            ),
          ),
        ],
