...
export const validatePayment = [
  body('tier').isIn(['free', 'premium', 'gold', 'platinum']),
...
export const validateFraudReport = [
  body('reportedUser').isMongoId(),
  body('reason').isIn(['fake_profile', 'scam', 'harassment', 'inappropriate_content', 'other']),
  body('details').trim().isLength({ min: 10 }),
  (req, res, next) => {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
      return res.status(400).json({ errors: errors.array() });
